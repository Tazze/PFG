public async Task InitializeCameraAsync() {
 ...
 if (_mediaCapture == null) {
  ...
  // Register for a notification when video recording has reached
  // the maximum time and when something goes wrong
  ...
  try {
   ...
   //Debug.WriteLine("Exception when initializing 
   MediaCapture with {0}: {1}", cameraDevice.Id, ex.ToString());
  }
  ...
  if (_isInitialized) {
   ...
   Windows.Devices.Enumeration.Panel.Unknown) {
    // No information on the location of the camera, 
    //assume it's an external camera, not integrated on the device
